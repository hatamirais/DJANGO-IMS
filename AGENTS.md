...
Stock rows and stock transactions carry `source_document_number`. Stock rows are uniquely identified by item, location, batch, funding source, and `source_document_number`; detailed reports and stock cards also group by that source layer. Receiving uses the receiving document number as the stock source document; opening balance uses the opening-balance document number. Receiving and opening-balance document numbers must not collide with each other because both become stock source-layer identifiers. Do not average unit prices across source documents. Same item/location/batch/funding can exist in separate source-document layers, while conflicts within the same source document must preserve exact expiry and unit price.
