...
- Initial stock bootstrap must use the Stock Admin opening-balance import at `/admin/stock/stock/opening-balance/import-csv/`, not `receiving.csv` or the direct `stock.csv` import. It is restricted to superuser / role `ADMIN`, uses `effective_date` for report classification, rejects populated `receiving_type` / `supplier_code`, and runs a validation/preview step before the explicit `Konfirmasi Import` write. Confirmed imports write `Transaction(IN, reference_type=INITIAL_IMPORT)`. Rekap/yearly reports classify these rows as `saldo_awal`; later years carry forward from the ledger and do not require re-import.
