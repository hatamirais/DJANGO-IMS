...
- Initial stock bootstrap must use the Stock Admin opening-balance import at `/admin/stock/stock/opening-balance/import-csv/`, not `receiving.csv` or the direct `stock.csv` import. The generic Stock admin import endpoint is disabled so stock cannot be written without ledger transactions. Opening-balance import is restricted to superuser / role `ADMIN`, uses one consistent `effective_date` per `document_number` for report classification, rejects populated `receiving_type` / `supplier_code`, validates decimal precision before preview/confirm, rejects negative unit prices, generates blank batches with document identity, and rejects existing-stock collisions with different `expiry_date` or `unit_price`. Confirmed imports write `Transaction(IN, reference_type=INITIAL_IMPORT)`. Rekap/yearly reports classify these rows as `saldo_awal`; later years carry forward from the ledger and do not require re-import.
