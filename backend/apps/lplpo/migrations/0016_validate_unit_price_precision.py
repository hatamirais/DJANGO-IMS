...
def validate_unit_price_precision(apps, schema_editor):
    model = apps.get_model("lplpo", "LPLPOItem")
    count = sum(
        1
        for value in model.objects.values_list("harga_satuan", flat=True).iterator()
        if _violates_price_precision(value)
    )
    if count:
        raise RuntimeError(
            "Cannot widen unit-price precision; existing values exceed 23,10: "
            f"LPLPOItem.harga_satuan: {count}"
        )


class Migration(migrations.Migration):
    dependencies = [
        ("lplpo", "0015_reconcile_model_drift"),
    ]
