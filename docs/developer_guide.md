...
- Untuk stok awal, gunakan import Saldo Awal admin-only di `/admin/stock/stock/opening-balance/import-csv/`; template kosong dapat diunduh dari `/admin/stock/stock/opening-balance/export-csv-template/`. Upload CSV menjalankan validasi dan pratinjau terlebih dahulu; data baru masuk ke sistem setelah admin menekan `Konfirmasi Import`. Import terkonfirmasi membuat `OpeningBalanceImport`, `OpeningBalanceImportItem`, `Stock`, dan `Transaction(IN, reference_type=INITIAL_IMPORT)` agar saldo awal tidak tercampur dengan penerimaan operasional. Import generik bawaan untuk model `Stock` dinonaktifkan supaya stok tidak dapat ditulis tanpa transaksi ledger.
- Import Saldo Awal dibatasi untuk superuser / role `ADMIN`, memakai `effective_date` sebagai tanggal efektif laporan, dan menolak nilai `receiving_type` atau `supplier_code` bila kolom tersebut ikut terbawa dari template penerimaan.
- Seluruh baris dalam satu `document_number` saldo awal harus memakai `effective_date` yang sama. `quantity` dan `unit_price` divalidasi sesuai presisi kolom tujuan, `unit_price` tidak boleh negatif, batch kosong dibuat dengan identitas dokumen, dan baris yang menabrak stok existing harus memiliki `expiry_date` serta `unit_price` yang sama.
