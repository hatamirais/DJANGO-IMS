...
- Seluruh baris dalam satu `document_number` saldo awal harus memakai `effective_date` yang sama. CSV Saldo Awal dapat memakai delimiter koma atau titik koma. `document_number` saldo awal tidak boleh sama dengan dokumen saldo awal yang sudah diposting atau dokumen receiving karena nomor tersebut menjadi identitas source layer stok; konfirmasi Saldo Awal dan pembuatan Receiving juga mengklaim nomor baru melalui `SourceDocumentNumberClaim` agar klaim lintas workflow terserialisasi di database. `quantity` divalidasi sesuai presisi kolom tujuan, sedangkan `unit_price` mendukung `max_digits=23` dan `decimal_places=10`, menerima koma desimal seperti `8893,31985`, dan tidak boleh negatif. Batch kosong dibuat dengan identitas dokumen, dan stok serta transaksi ledger disimpan sebagai layer `item + lokasi + batch + sumber dana + document_number`. Konflik `expiry_date` atau exact `unit_price` ditolak dalam layer dokumen sumber yang sama; batch yang sama dari dokumen sumber berbeda tetap dipisah dan tidak dirata-ratakan.
