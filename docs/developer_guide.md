...
- Untuk stok awal, gunakan import Saldo Awal admin-only di `/admin/stock/stock/opening-balance/import-csv/`; template kosong dapat diunduh dari `/admin/stock/stock/opening-balance/export-csv-template/`. Import ini membuat `OpeningBalanceImport`, `OpeningBalanceImportItem`, `Stock`, dan `Transaction(IN, reference_type=INITIAL_IMPORT)` agar saldo awal tidak tercampur dengan penerimaan operasional.
