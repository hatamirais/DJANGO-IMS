...
- Seluruh baris dalam satu `document_number` saldo awal harus memakai `effective_date` yang sama. CSV Saldo Awal dapat memakai delimiter koma atau titik koma. `document_number` saldo awal tidak boleh sama dengan dokumen saldo awal yang sudah diposting atau dokumen receiving karena nomor tersebut menjadi identitas source layer stok. `quantity` dan `unit_price` divalidasi sesuai presisi kolom tujuan, `unit_price` tidak boleh negatif, batch kosong dibuat dengan identitas dokumen, dan stok serta transaksi ledger disimpan sebagai layer `item + lokasi + batch + sumber dana + document_number`. Konflik `expiry_date` atau `unit_price` ditolak dalam layer dokumen sumber yang sama; batch yang sama dari dokumen sumber berbeda tetap dipisah dan tidak dirata-ratakan.
...
- Nomor dokumen receiving juga divalidasi terhadap dokumen Saldo Awal yang sudah diposting, dan nomor receiving otomatis akan melewati nilai `RCV-YYYY-NNNNN` yang sudah dipakai Saldo Awal agar identitas source layer tidak bertabrakan lintas workflow.
