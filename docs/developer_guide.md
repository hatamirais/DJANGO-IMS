...
- Seluruh baris dalam satu `document_number` saldo awal harus memakai `effective_date` yang sama. CSV Saldo Awal dapat memakai delimiter koma atau titik koma. `document_number` saldo awal tidak boleh sama dengan dokumen receiving atau klaim sumber non-saldo-awal karena nomor tersebut menjadi identitas source layer stok; konfirmasi Saldo Awal dan pembuatan Receiving juga mengklaim nomor baru melalui `SourceDocumentNumberClaim` agar klaim lintas workflow terserialisasi di database. `document_number` saldo awal yang sudah diposting boleh dipakai ulang untuk melengkapi import parsial: baris dengan layer stok yang sudah persis sama dilewati, sedangkan baris baru diposting ke header import yang sudah ada. `batch_lot` wajib diisi saat reimport dokumen saldo awal yang sudah diposting karena nama batch otomatis untuk nilai kosong memakai nomor baris CSV. `quantity` divalidasi sesuai presisi kolom tujuan, sedangkan `unit_price` mendukung `max_digits=23` dan `decimal_places=10`, menerima koma desimal seperti `8893,31985`, dan tidak boleh negatif. Batch kosong pada import pertama dibuat dengan identitas dokumen, dan stok serta transaksi ledger disimpan sebagai layer `item + lokasi + batch + sumber dana + document_number`. Konflik `expiry_date` atau exact `unit_price` ditolak dalam layer dokumen sumber yang sama; batch yang sama dari dokumen sumber berbeda tetap dipisah dan tidak dirata-ratakan.
...
- Kolom opsional `receiving_type` pada import penerimaan default ke `GRANT` bila tidak diisi; jangan gunakan default ini untuk saldo awal karena stok bootstrap sudah memiliki import khusus.
