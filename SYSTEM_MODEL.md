...
- Procurement contract cancellation is a soft-cancel workflow, not a delete. `DRAFT` and `SUBMITTED` SPJ can be cancelled with a required reason. `APPROVED` SPJ can be cancelled only while the linked planned receiving has no `ReceivingItem` rows and no received quantity; unused linked receiving plans are marked `CANCELLED` with the same reason/user/time. Once procurement receiving has any realization, cancellation is blocked and users must use amendment/close workflow.
